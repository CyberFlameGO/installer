...
public class Installer.MainWindow : Gtk.Dialog {
    ...
    private bool is_refresh() {
        try {
            return this.distinst.is_refresh();
        } catch (Error e) {
            warning("failed to check if in refresh environment: %s", e.message);
            return false;
        }
    }
        ...
        this.disk_rescan_signal = this.distinst.disk_rescan_complete.connect(() => {
            ...
            Timeout.add(100, () => {
                ...
                if (this.refresh_view == null) {
                    ...
                    this.refresh_view.cancel.connect (load_try_install_view);
    ...
    private void load_try_install_view () {
        ...
        try_install_view.refresh_step.connect (() => {
            this.os_search();
        });
    }
